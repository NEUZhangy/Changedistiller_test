...
public class MultiClass_2 {
    ...
    public MultiClass_2(String str, String type) {
        args = str.split(" ");
        methodT = type;
    }
     ...
     * @throws IOException
     * @see Slicer.DataDependenceOptions <li>"control dependence options" can be "-full" or "-none" <li>the
    ...
    public Process run(String appJar, String mainClass, String srcCaller, String srcCallee, boolean goBackward,
            ...
            IOException {
        try {
            System.out.println("This is for Analysis the Static Parameter: ---------------------------------------");
...
//                        System.out.println("\t" +"SSAGETinstruction " + inst + " v:" + value);
    ...
    //TODO: find the keystatement; methodType 是函数类型。
        ...
        for(int i = listArr.size()-1; i>=0; i--) {
            ...
            if(stm.getKind()== Statement.Kind.PARAM_CALLEE){
                ...
                while(it.hasNext()) {
                    ...
                    if (varuse < 0) {paraMap.remove(n); continue;}
                    ...
                    if(st.isConstant(use)){
                        ...
                        bits.add(use); // not constant, should be passin parameter again, add to use
                ...
                if(stm.equals(keyStatement) || definsts.contains(inst)
